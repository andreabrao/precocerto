...
export async function POST(request: Request) {
  try {
    ...
    if (!contributorId || !productId || typeof targetCents !== "number" || !Number.isInteger(targetCents) || targetCents <= 0) {
    ...
    await savePriceAlertPreference(db, contributorId, productId, targetCents);
