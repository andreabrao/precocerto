...
export async function POST(request: Request) {
  ...
  if (request.headers.get("x-import-key") !== configuredKey) {
  ...
  try {
    ...
    if (!storeId || !productId || !Number.isInteger(payload.priceCents) || (payload.priceCents ?? 0) <= 0 || !isKnownSourceType || !validHttpsUrl(payload.sourceUrl) || (requiresSourceUrl && !payload.sourceUrl)) {
