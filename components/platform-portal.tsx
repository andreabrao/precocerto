...
export function PlatformPortal({ onOpenShopping, onClose }: { onOpenShopping: () => void; onClose: () => void }) {
  ...
  if (view === "landing") return <section className="platform-landing" aria-label="Apresentação do PreçoCerto">
    ...
    <div className="landing-brand"><span>p</span> preçocerto</div>
    <div className="landing-content"><p className="eyebrow">INTELIGÊNCIA LOCAL DE PREÇOS</p><h1>Ofertas confiáveis.<br /><em>Decisões melhores.</em></h1><p>Uma plataforma para consumidores economizarem e para varejistas entenderem a região, publicarem ofertas e acompanharem resultados.</p><div className="landing-actions"><button onClick={() => setView("access")}>Entrar na plataforma <span>→</span></button><button className="landing-secondary" onClick={onOpenShopping}>Explorar ofertas</button></div></div>
    <div className="landing-columns"><article><b>Para clientes</b><span>Compare preços, monte a cesta e receba alertas.</span></article><article><b>Para mercados</b><span>Publique folhetos e acompanhe ofertas da sua região.</span></article><article><b>Para gestão</b><span>Valide dados com IA e mantenha a base confiável.</span></article></div>
  </section>;
